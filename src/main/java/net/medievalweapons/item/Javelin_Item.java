...
public class Javelin_Item extends SwordItem
{
    ...
    public Javelin_Item(Tier toolMaterial, float attackDamage, float attackSpeed, Supplier<EntityType<Javelin_Entity>> typeSupplier, Properties settings)
    {
        ...
        if (user instanceof Player)
        {
            ...
            if (i >= 10)
            {
                if (!world.isClientSide)
                {
                    ...
                    Javelin_Entity.shootFromRotation(playerEntity, playerEntity.getXRot(), playerEntity.getYRot(), 0.0F, 2.5F, 1.0F);
