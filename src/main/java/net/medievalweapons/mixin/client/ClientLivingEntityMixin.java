...
public class ClientLivingEntityMixin {
    ...
    private void handleStatusMixin(byte status, CallbackInfo info) {
        ...
        if (itemStack.is(TagInit.ACCROSS_DOUBLE_HANDED_ITEMS) || itemStack.getItem() instanceof Big_Axe_Item) {
            livingEntity.playSound(SoundInit.PARRYING_EVENT, 1.0F, 0.9F + livingEntity.level.random.nextFloat() * 0.2F);
            info.cancel();
        } else if (itemStack.is(TagInit.DOUBLE_HANDED_ITEMS) || itemStack.getItem() instanceof Long_Sword_Item) {
            livingEntity.playSound(SoundInit.SWORD_PARRYING_EVENT, 1.0F, 0.9F + livingEntity.level.random.nextFloat() * 0.2F);
            info.cancel();
        }
    }
}
